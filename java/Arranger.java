...
public class Arranger
{
	...
	public boolean inOrder(ArrayList<String> arr)
	{
		...
		{
			if(str.compareTo(first) < 0)		 //  TO BE COMPLETED BY YOU.
			{
				return false;
			}
			first = str;
	...
	public void swap(ArrayList<String> arr, int i, int j)
	{
		...
		arr.set(i, arr.get(j));
		arr.set(j, temp);//  TO BE COMPLETED BY YOU.
	}
	...
	{
		for (int i = 0; i < arr.size(); i++)		//  TO BE COMPLETED BY YOU.
		{
	...
	public void rotateLeft(ArrayList<Integer> a)
	{
		...
		a.set(a.size() - 1, temp);                            //  TO BE COMPLETED BY YOU.
	}
